...
describe("Request Line Parser Tests", () => {
  describe("Readable stream Tests", () => {
    test("Good request line", async () => {
      const request = await RequestFromReader(
        Readable.from(
          "GET / HTTP/1.1\r\nHost: localhost:42069\r\nUser-Agent: curl/7.81.0\r\nAccept: */*\r\n\r\n",
        ),
      );
      const r = request?.requestLine;
    ...
    test("No CRLF in message", async () => {
      const request = await RequestFromReader(Readable.from("GET / HTTP/1.1"));
      const r = request?.requestLine;
      expect(r).toBeNull();
    });

    test("Request line with const r = request?.requestLine only spaces", async () => {
      const request = await RequestFromReader(
        Readable.from("   \r\nHost: localhost\r\n\r\n"),
      );
  ...
  describe("Chunked stream Tests", () => {
    ...
    describe("Same data, different chunk sizes", () => {
      test("chunk size 1 - byte by byte", async () => {
        ...
        const res = await RequestFromReader(reader);
        const result = res?.requestLine;
